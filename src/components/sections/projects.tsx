...
import { ChevronLeft, ChevronRight, Github, ExternalLink, ChevronDown, ChevronUp, ImageIcon, CalendarDays } from 'lucide-react'
...
const ProjectCard = React.memo(({ project }: { project: typeof projects.items[0] }) => {
  const [isExpanded, setIsExpanded] = useState(false)
  const [showImage, setShowImage] = useState(false)

  const shortDescription = project.description.split(' ').slice(0, 20).join(' ') + '... Click me';
      ...
      >
        <CardHeader className="p-3">
          <div className="flex items-center justify-between">
            ...
            <CardTitle className="text-sm">$ cat {project.title.toLowerCase().replace(/ /g, '_')}.md</CardTitle>
            {project.date && (
