...
import { terminalContent } from '@/config/terminal-content';
...
const commands: Record<string, Command> = Object.entries(
  terminalContent.commands
).reduce(
  (acc, [key, value]) => {
    acc[key] = {
      ...value,
      action: new Function(
        'args',
        `
        const commands = ${JSON.stringify(terminalContent.commands)};
        return (async function() {
          ${value.action}
        })(args);
      `
      ) as (args: string[]) => Promise<string>,
    };
    return acc;
  },
...
const generateUserId = () => {
  ...
  const existingUserIds = savedUserData
    ? Object.keys(JSON.parse(savedUserData))
    : [];
  let newId = 1;
...
const TerminalPrompt: React.FC<{ currentDirectory: string }> = React.memo(
  ({ currentDirectory }) => (
    <>
...
const TerminalOutput: React.FC<{ output: string[] }> = React.memo(
  ({ output }) => (
    <>
    ...
    (e: React.KeyboardEvent<HTMLInputElement>) => {
      ...
      if (e.key === 'ArrowUp') {
        ...
        if (historyIndex < currentUserData.commandHistory.length - 1) {
          ...
          setInput(
            currentUserData.commandHistory[
              currentUserData.commandHistory.length - 1 - historyIndex - 1
            ]
          );
        }
    ...
    <>
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <div
              className={`container mx-auto h-full flex items-center justify-center ${isFullScreen ? 'p-0' : ''}`}
            >
              <motion.div
